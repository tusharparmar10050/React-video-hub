...
const Videos = () => {
    ...
    const videoshandler = (item, index) => {
        setVideoSrc(item)
    ...
    return (<Stack direction={['column', 'row']} h={'100vh'}>
        <VStack w={'full'}>
            ...
            <VStack alignItems={'flex-start'} p={'8'} w={'full'} overflowY={'auto'}>
                <Heading>Sample Video {videoIndex}</Heading>
                <Text>
                    This is a sample video fro testing and demo. This is called description.
                </Text>
            </VStack>
        </VStack>

        <VStack w={['full', 'md']} alignItems={'stretch'} p="8" spacing={"8"} overflow={'auto'}>
            {
                videosArr.map((item, index) => (
                    <Button variant={'ghost'} colorScheme={'purple'} onClick={() => videoshandler(item, index)}>
                        Lecture {index + 1}
                    </Button>
                )
                )
